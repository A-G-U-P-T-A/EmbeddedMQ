...
emq_status emq_push(emq_queue *q, const void *data, size_t size,
  ...
  if (emq_desc_spsc_lfq(q->desc) && q->desc->opts.policy != EMQ_POLICY_DELAY) {
    /* FAST SPSC: consumers use emq_pop directly — skip scheduler activate. */
...
emq_status emq_peek(emq_queue *q, emq_message *out) {
  ...
  if (rc == 0) {
    /* Borrowed pointer into log/scratch — must not free. */
    out->flags |= EMQ_MSG_FLAG_BORROWED;
  }
  emq_mutex_unlock((emq_mutex *)q->desc->op_mu_opaque);
  return emq_map_rc(rc);
}

emq_status emq_claim(emq_queue *q, emq_message *out, uint32_t timeout_ms) {
  uint64_t deadline;
  uint64_t head = 0;
  const void *ptr = NULL;
  uint32_t len = 0, flags = 0, prio = 0;
  uint64_t msg_id = 0;
  int rc;
  if (!q || !q->desc || !out || !q->desc->lfq) return EMQ_ERR_INVALID;
  deadline = timeout_ms == 0 ? 0
                             : emq_now_ns() + (uint64_t)timeout_ms * 1000000ULL;
  for (;;) {
    uint64_t expect =
        emq_atomic_load_u64((emq_atomic_u64 *)&q->desc->seq_wait);
    rc = emq_lfq_try_claim(q->desc->lfq, &ptr, &len, &msg_id, &flags, &prio,
                           &head);
    if (rc == 0) {
      memset(out, 0, sizeof(*out));
      out->id = msg_id;
      out->priority = prio;
      out->flags = flags | EMQ_MSG_FLAG_CLAIMED;
      out->data = ptr;
      out->size = len;
      out->offset = head; /* stash claim head for release */
      return EMQ_OK;
    }
    if (timeout_ms == 0) return EMQ_ERR_EMPTY;
    {
...
emq_status emq_release_claim(emq_queue *q, emq_message *message) {
  int rc;
  if (!q || !q->desc || !message || !q->desc->lfq) return EMQ_ERR_INVALID;
  if ((message->flags & EMQ_MSG_FLAG_CLAIMED) == 0) return EMQ_ERR_INVALID;
  rc = emq_lfq_release_claim(q->desc->lfq, message->offset,
                             (uint32_t)message->size);
  if (rc == 0) {
    q->desc->stats.dequeued++;
    q->desc->stats.depth =
        emq_atomic_fetch_add_u64(&q->desc->hot.depth, (uint64_t)-1) - 1;
...
emq_status emq_push_async(emq_queue *q, const void *data, size_t size,
  ...
  if (q->rt->engine.worker_count == 0) return EMQ_ERR_UNSUPPORTED;
...
void emq_message_release(emq_message *message) {
  if (!message) return;
  /* Peek (borrowed) and claim (ring-owned) must not free payload here. */
  if ((message->flags & (EMQ_MSG_FLAG_BORROWED | EMQ_MSG_FLAG_CLAIMED)) == 0 &&
      message->data) {
