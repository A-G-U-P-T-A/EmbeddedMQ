...
static int emq_lfq_reserve(emq_lfq *q, uint32_t need, uint64_t *out_offset) {
  for (;;) {
    ...
    if (q->spsc) {
      /* Single producer: plain store, no locked RMW. */
      frame_off = tail + pad;
      if (pad > 0) emq_lfq_write_padding(q, tail, pad);
      emq_atomic_store_release_u64(&q->tail, tail + claim_bytes);
...
int emq_lfq_try_claim(emq_lfq *q, const void **out_ptr, uint32_t *out_len,
  ...
  if (!q || !out_ptr || !out_len || !out_head) return -1;
  ...
  for (;;) {
    ...
    total = emq_lfq_frame_total((uint32_t)len);
    (void)total;
    *out_ptr = (const void *)((uint8_t *)f + EMQ_LFQ_FRAME_HDR);
    *out_len = (uint32_t)len;
    if (out_msg_id) *out_msg_id = f->msg_id;
    if (out_flags) *out_flags = f->flags;
    if (out_priority) *out_priority = f->priority;
