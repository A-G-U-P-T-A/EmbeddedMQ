...
/* Successful pop/sub-next calls transfer message.data ownership to the caller.
 * Release it with emq_message_release(). */
...
/* Async callbacks execute on runtime workers. */
...
/* Stackless task substrate (protothread-style). Opaque handle; line field
 * is accessible for EMQ_TASK_* macros via emq_task_line(). */
