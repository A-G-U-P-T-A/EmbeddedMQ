...
/* Layout freeze for 64-bit ABIs. Update deliberately when changing public types. */
...
EMQ_STATIC_ASSERT(offsetof(emq_message, data) > offsetof(emq_message, ttl_ns),
                  msg_data_after_ttl);
...
/* Link-completeness: referencing every public symbol. */
